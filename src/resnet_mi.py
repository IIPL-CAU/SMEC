...
# class ie_skip(nn.Module):
#     def __init__(self,config):
#         super(ie_skip,self).__init__()
#         self.T=5
#         self.wd=0.05
#         self.graph=False
...
class Bottleneck(nn.Module):
    ...
    def forward(self, x):
        ...
        layer=nn.sequential(self.conv1,
                            self.bn1,
                            self.relu,
                            self.conv2,
                            self.bn2,
        ...
        if self.ie_skips:
            for i in range(1):
                with torch.set_grad_enabled(True):
                    loss_output=torch.norm(out-x-layer(out))**2
                    f_output=torch.autograd.grad(loss_output, out, retain_graph=self.graph)
                    out=out-0.05*f_output[0]
